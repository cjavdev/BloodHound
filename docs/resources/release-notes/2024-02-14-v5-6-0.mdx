...
* **New "Power User" role -** BloodHound now includes an additional role as a bridge between "User" and "Administrator" for customers with users who should be able to modify the graph through file uploads or modifications of collectors but who should not be able to modify who can access the environment. For more, see [Administering users and roles.](/resources/users-and-roles)
