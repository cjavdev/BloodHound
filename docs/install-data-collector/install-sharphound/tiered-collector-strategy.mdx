...
* SharpHound Enterprise **can** be hardened to mitigate the risk of Kerberos Delegation and NTLM authentication, see [SharpHound Enterprise Service Hardening](/hc/en-us/articles/SharpHound-Enterprise-Service-Hardening).
...
* Creation of one BloodHound collector server for each tier, see [SharpHound Enterprise System Requirements](/hc/en-us/articles/SharpHound-Enterprise-System-Requirements-and-Deployment-Process)
    * Tip: In BloodHound, mark the Tier Zero collector server as Tier Zero, see [Modifying Tier Zero](/hc/en-us/articles/Modifying-Tier-Zero)
* Creation of one SharpHound Enterprise service account for each tier, see [Create a gMSA for use with SharpHound Enterprise](/hc/en-us/articles/Create-a-gMSA-for-use-with-SharpHound-Enterprise)
    * Each service account must be a member of the local Administrators group on all systems in the service account's tier.
    * Each service account is recommended to be hardened, see [SharpHound Enterprise Service Hardening](/hc/en-us/articles/SharpHound-Enterprise-Service-Hardening).
    * Tip: In BloodHound, mark the Tier Zero service account as Tier Zero, see [Modifying Tier Zero](/hc/en-us/articles/Modifying-Tier-Zero)
...
1.  Follow the article [Create a SharpHound Enterprise collector client](/hc/en-us/articles/Create-a-BloodHound-Enterprise-collector-client)
...
2.  Install the collector client on the dedicated Tier Zero BloodHound collector server, using the dedicated Tier Zero SharpHound Enterprise service account. [See Install and Upgrade SharpHound Enterprise](/hc/en-us/articles/Install-and-Upgrade-SharpHound-Enterprise).
...
1.  On the Tier Zero collector client, create a new collection schedule, see [Create a data collection schedule](/hc/en-us/articles/Create-a-data-collection-schedule)
